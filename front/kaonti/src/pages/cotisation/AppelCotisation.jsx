...
const MyPdfDocument = ({ data, tabValue, exerciceLabel }) => {
  // Calcul des totaux
...
const AppelCotisation = () => {
    ...
    const fetchDataInit = async () => {
      try {
        const [resEx, resMem] = await Promise.all([
          ...
          //axiosPrivate.get('/api/membres/active')
        ]);
  ...
  const columns = [
    ...
    { field: 'statut', headerName: 'Statut', width: 120, renderCell: (p) => renderBadge(p, { 'Expert Comptable': '#0288d1', 'Expert Stagiaire': '#455a64' }) },
    ...
    { field: 'valide', headerName: 'État', width: 100, renderCell: (p) => ( <Chip label={p.value ? "Validé" : "Brouillon"} size="small" color={p.value ? "success" : "warning"} variant="outlined" sx={{ fontWeight: 700 }} /> ) },
    {
      ...
      renderCell: (params) => (
        <Stack direction="row" spacing={1}>
          ...
          {tabValue === 0? <PDFDownloadLink 
              document={<TicketNotePDF row={params.row} exercice={exercices}/>} 
              fileName={`Note_${params.row.nom}.pdf`}
    ...
    <Box sx={{ p: 0, bgcolor: '#f8fafc', minHeight: '100vh' }}>
          ...
          <Box sx={{ 
              ...
              <Stack direction="row" spacing={4} sx={{ mb: 0.5 }}>
                  {[
                      ...
                      { label: 'AJUSTEMENTS', val: calculateTotalAjustement(), color: '#64748b' },
                      { label: 'APPEL NET', val: calculateTotalAppelNet(), color: '#2563EB' }
                  ].map((item, index) => (
      ...
      <Box sx={{ width: '100%' }}>
        ...
        <Stack direction="row" spacing={2} sx={{ mb: 2 }}>
          ...
          <PDFDownloadLink 
            document={<MyPdfDocument 
                      ...
                      />} 
            fileName={`Export_${tabValue === 0 ? 'Appels' : 'Ajustements'}.pdf`}
            style={{ textDecoration: 'none' }}
          >
            {({ loading }) => (
              <Button 
        ...
        <Paper elevation={0} sx={{ borderRadius: '16px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
          ...
          <Box sx={{minHeight: '40vh', maxHeight: '40vh'}}>
            <DataGrid 
              ...
              rowHeight={38} 
              sx={{ border: 'none', '& .MuiDataGrid-columnHeaders': { bgcolor: '#f8fafb', fontWeight: 800 } }} 
            />
          </Box>
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12}>
              ...
              <TextField 
                ...
                // On affiche la valeur formatée avec des espaces
                value={formatNumber(formAjust.montant)}
                onChange={(e) => {
                  // On retire tous les espaces pour ne stocker que le nombre pur dans le state
                ...
                InputProps={{
                  endAdornment: <Typography variant="caption" sx={{ fontWeight: 700, color: '#94a3b8', ml: 1 }}>AR</Typography>,
                }}
                sx={{ 
                  '& .MuiOutlinedInput-root': { bgcolor: '#f1f5f9', border: 'none', fontWeight: 700 } 
                }}
              />
            </Grid>
            )}
            ...
            <Grid item xs={6}>
              <InputLabel sx={{ mb: 0.5, fontWeight: 700, fontSize: '0.75rem', color: '#64748b', textTransform: 'uppercase' }}>Statut</InputLabel>
              <TextField select fullWidth size="small" value={formAjust.statut} onChange={(e) => setFormAjust({...formAjust, statut: e.target.value})} sx={{ '& .MuiOutlinedInput-root': { bgcolor: '#f1f5f9' } }}>
                <MenuItem value="Toutes">Tous les statuts</MenuItem>
                <MenuItem value="Expert Comptable">Expert Comptable</MenuItem>
