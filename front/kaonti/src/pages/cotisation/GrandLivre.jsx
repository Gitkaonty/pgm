...
import { DataGrid, gridClasses, GridToolbar, useGridApiContext, GridFooterContainer, GridFooter } from '@mui/x-data-grid';
    ...
    <Document>
        ...
        {options.balance && (
            <Page size="A4" orientation="landscape" style={pdfStyles.page}>
                ...
                <View fixed style={pdfStyles.headerContainer}>
                ...
                <View style={pdfStyles.table}>
                    <View fixed style={[pdfStyles.tableRow, pdfStyles.header]}>
                ...
                <View fixed style={pdfStyles.footerContainer}>
                    <View style={pdfStyles.footerLine} />
                    <View style={pdfStyles.footerContent}>
                        <Text style={pdfStyles.footerBranch}>OECFM - Tableau balances générales</Text>
                        <Text
        ...
        {options.grandLivre && (
            <Page size="A4" orientation="landscape" style={pdfStyles.page}>
                ...
                <View fixed style={pdfStyles.footerContainer}>
                    <View style={pdfStyles.footerLine} />
                    <View style={pdfStyles.footerContent}>
                        <Text style={pdfStyles.footerBranch}>OECFM - Tableau Grand livre</Text>
                        <Text
                            style={pdfStyles.pageNumber}
                            render={({ pageNumber, totalPages }) => (
                                `Page ${pageNumber} / ${totalPages}`
...
const GrandLivrePage = () => {
    ...
    const CustomFooter = () => {
        ...
        // Calculs
        const totalAnouveau = visibleRows.reduce((sum, row) => sum + (Number(row?.soldeInitial) || 0), 0);
        const totaldebit = visibleRows.reduce((sum, row) => sum + (Number(row?.totalDebit) || 0), 0);
        const totalcredit = visibleRows.reduce((sum, row) => sum + (Number(row?.totalCredit) || 0), 0);
        const totalsolde = visibleRows.reduce((sum, row) => sum + (Number(row?.solde) || 0), 0);
            ...
            <GridFooterContainer sx={{ p: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center', bgcolor: '#f8fafb' }}>
                <Box sx={{ display: 'flex', gap: 3, ml: 2 }}>
                    ...
                    <Typography variant="caption">
                        Total anouveaux: <strong>{fNum(totalAnouveau)} Ar</strong>
                    </Typography>
                    <Typography variant="caption">
                        Total débit: <strong>{fNum(totaldebit)} Ar</strong>
                    </Typography>
                    <Typography variant="caption">
                        Total crédit: <strong>{fNum(totalcredit)} Ar</strong>
                    </Typography>
                    <Typography variant="caption">
                        Solde: <strong>{fNum(totalsolde)} Ar</strong>
                    </Typography>
                </Box>
                <GridFooter />
            </GridFooterContainer>
