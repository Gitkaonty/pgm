...
const PdfAttestationExpA = ({ rows, infoSignataire, withSignature, qrDataUrl }) => {
    ...
    // La signature électronique n'est affichée que si demandée ET l'attestation validée
    const showSig = withSignature && rows.validation_1 && rows.validation_2;
    ...
    return (
        <Document>
            <Page size="A4" style={{ padding: 40, fontSize: 10, fontFamily: 'Helvetica' }}>
                ...
                {qrDataUrl && (
                    <View style={{ position: 'absolute', top: 152, right: 45, alignItems: 'center' }}>
                ...
                {/* Signatures — emplacement de hauteur fixe : même distance avec ou sans signature
                    (sans signature, l'espace reste libre pour signer à la main) */}
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 20, marginHorizontal: 50 }}>
                    <View style={{ flexDirection: 'column', alignItems: 'center' }}>
                        <Text style={{ marginBottom: showSig ? '-20px' : 0 }}>
                        ...
                        <Text style={{ marginTop: showSig ? '-50px' : 60 }}>
                            {nom_secretaire}
                        </Text>
                    </View>
